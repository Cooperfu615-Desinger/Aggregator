...
### D. 表格標準行為 (Table Standard Behaviors)

所有 `<n-data-table>` 列表頁面必須統一實作以下互動規範：

1. **全面支援排序 (Global Sorting)**:
    - 凡是數值 (ID, Amount, %), 狀態 (Status), 或關鍵字串 (Code, Account) 欄位，**必須** 開啟 `sorter` 功能。
    - 禁止僅顯示靜態表格。

2. **標準排序邏輯 (Sorter Implementation)**:
    - **String**: 使用 `(a, b) => a.key.localeCompare(b.key)` (確保中文/英文排序正常)。
    - **Number**: 使用 `(a, b) => a.key - b.key` (嚴禁將數字當字串排，避免 '10' 排在 '2' 前面)。
    - **Boolean/Enum**: 使用 `Number(a.status) - Number(b.status)`。
